...
#ifndef __THREAD_FLOW_CONTROL_H__
#define __THREAD_FLOW_CONTROL_H__
...
_thread_flow_control(
	...
	ub *memory_use_percentage,
	ub *coroutines_site_creat_counter, ub *coroutines_site_release_counter, ub *site_counter)
{
...
{
	ub memory_use_percentage, coroutines_site_creat_counter, coroutines_site_release_counter, site_counter;

	dave_bool ret = _thread_flow_control(
		pThread,
		&memory_use_percentage,
		&coroutines_site_creat_counter, &coroutines_site_release_counter, &site_counter);

	if(ret == dave_false)
	{
		THREADLTRACE(3, 1,
			"Service %s at flow control! memory(sys:%ld cfg:%ld) coroutines(creat:%ld release:%ld max number of coroutine:%ld/%ld)",
			pThread->thread_name,
