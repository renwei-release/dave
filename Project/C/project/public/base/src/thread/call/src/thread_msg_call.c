...
static inline void
_thread_msg_call_add(ThreadId thread_id, ub msg_id, base_thread_fun msg_fun, void *user_ptr)
{
	MsgCallFun *pFun;
	s8 key[256];
...
static inline void
_thread_msg_call_pre(void)
{
	static volatile sb __safe_pre_flag__ = 0;

	SAFEPre(__safe_pre_flag__, { _msg_call_kv = kv_malloc("tmckv", 0, NULL); });
}
