...
{
  size_t datalen = strlen(data) * 3 + 16;
  char* result;
  if ((result = (char*)malloc(datalen)) == NULL) {
    DEBUG_ERROR(ERRMSG_MEMORY_ALLOCATION_ERROR)
    return NULL;
  }

  snprintf(result, datalen, "<%s>", data);
...
const char* quickmail_protocol_send (quickmail mailobj, const char* smtpserver, unsigned int smtpport, int protocol, const char* username, const char* password)
{
  ...
  if ((curl = curl_easy_init()) != NULL) {
    ...
    //set destination URL
    char* addr;
    size_t len = strlen(smtpserver) + 15;
    if ((addr = (char*)malloc(len)) == NULL) {
      DEBUG_ERROR(ERRMSG_MEMORY_ALLOCATION_ERROR)
      return ERRMSG_MEMORY_ALLOCATION_ERROR;
    }
	if(smtpport == 0)
		snprintf(addr, len, "%s", smtpserver);
	else
    	snprintf(addr, len, "%s:%u", smtpserver, smtpport);
