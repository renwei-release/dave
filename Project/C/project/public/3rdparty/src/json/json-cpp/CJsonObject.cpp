/*
 * ================================================================================
 * (c) Copyright 2021 wulei All rights reserved.
 * --------------------------------------------------------------------------------
 * 2021.01.20.
 * ================================================================================
 */
...
{
    // m_pJsonData = _cJSON_CreateObject();  
...
bool CJsonObject::AddEmptySubObject(const std::string& strKey)
{
    ...
    if (pFocusData->type != _cJSON_Object)
    {
        m_strErrMsg = "not a json object! json array?";
        return(false);
    }
    if (_cJSON_GetObjectItem(pFocusData, strKey.c_str()) != NULL)
    {
        m_strErrMsg = "key exists!";
        return(false);
    }
    _cJSON* pJsonStruct = _cJSON_CreateObject();
    if (pJsonStruct == NULL)
    {
        m_strErrMsg = std::string("create sub empty object error!");
        return(false);
    }
    _cJSON_AddItemToObject(pFocusData, strKey.c_str(), pJsonStruct);
...
bool CJsonObject::AddEmptySubArray(const std::string& strKey)
{
    ...
    _cJSON* pJsonStruct = _cJSON_CreateArray();
    if (pJsonStruct == NULL)
    {
...
CJsonObject& CJsonObject::operator[](const std::string& strKey)
{
    ...
    {
        _cJSON* pJsonStruct = NULL;
        if (m_pJsonData != NULL)
        {
            if (m_pJsonData->type == _cJSON_Object)
            {
                pJsonStruct = _cJSON_GetObjectItem(m_pJsonData, strKey.c_str());
            }
        }
        else if (m_pExternJsonDataRef != NULL)
        {
            if (m_pExternJsonDataRef->type == _cJSON_Object)
            {
                pJsonStruct = _cJSON_GetObjectItem(m_pExternJsonDataRef, strKey.c_str());
            }
        }
        if (pJsonStruct == NULL)
        {
...
CJsonObject& CJsonObject::operator[](unsigned int uiWhich)
{
    ...
    if (iter == m_mapJsonArrayRef.end())
    {
        ...
        if (m_pJsonData != NULL)
        {
            if (m_pJsonData->type == _cJSON_Array)
            {
                pJsonStruct = _cJSON_GetArrayItem(m_pJsonData, uiWhich);
            }
        }
        else if (m_pExternJsonDataRef != NULL)
        {
            if (m_pExternJsonDataRef->type == _cJSON_Array)
            {
                pJsonStruct = _cJSON_GetArrayItem(m_pExternJsonDataRef, uiWhich);
            }
        }
        if (pJsonStruct == NULL)
        {
...
std::string CJsonObject::operator()(const std::string& strKey) const
{
    ...
    if (pJsonStruct->type == _cJSON_String)
    {
        return(pJsonStruct->valuestring);
    }
    else if (pJsonStruct->type == _cJSON_Int)
    {
        char szNumber[128] = {0};
    ...
    else if (pJsonStruct->type == _cJSON_Double)
    {
        char szNumber[128] = {0};
    ...
    else if (pJsonStruct->type == _cJSON_False)
    {
        return(std::string("false"));
    }
    else if (pJsonStruct->type == _cJSON_True)
    {
...
{
    Clear();
    m_pJsonData = _cJSON_Parse(strJson.c_str(), &mc_pError);
    m_pKeyTravers = m_pJsonData;
    if (m_pJsonData == NULL)
    {
...
bool CJsonObject::IsArray() const
{
    ...
    if (pFocusData->type == _cJSON_Array)
    {
        return(true);
    }
    else
...
std::string CJsonObject::ToString() const
{
    ...
    if (m_pJsonData != NULL)
    {
        pJsonString = _cJSON_PrintUnformatted(m_pJsonData);
    }
    else if (m_pExternJsonDataRef != NULL)
    {
        pJsonString = _cJSON_PrintUnformatted(m_pExternJsonDataRef);
    }
    if (pJsonString != NULL)
    {
...
std::string CJsonObject::ToFormattedString() const
{
    ...
    if (m_pJsonData != NULL)
    {
        pJsonString = _cJSON_Print(m_pJsonData);
    }
    else if (m_pExternJsonDataRef != NULL)
    {
        pJsonString = _cJSON_Print(m_pExternJsonDataRef);
    }
    if (pJsonString != NULL)
    {
        strJsonData = pJsonString;
...
bool CJsonObject::Get(const std::string& strKey, CJsonObject& oJsonObject) const
{
    ...
    char* pJsonString = _cJSON_Print(pJsonStruct);
...
bool CJsonObject::Get(const std::string& strKey, int32& iValue) const
{
    ...
    if (pJsonStruct->type == _cJSON_Int)
    {
...
bool CJsonObject::Get(const std::string& strKey, bool& bValue) const
{
    ...
    if (pJsonStruct->type > _cJSON_True)
    {
        return(false);
    }
...
bool CJsonObject::Get(const std::string& strKey, float& fValue) const
{
    ...
    if (pJsonStruct->type == _cJSON_Double || pJsonStruct->type == _cJSON_Int)
    {
...
bool CJsonObject::IsNull(const std::string& strKey) const
{
    ...
    if (pJsonStruct->type != _cJSON_NULL)
    {
        return(false);
    }
    return(true);
...
bool CJsonObject::Add(const std::string& strKey, const CJsonObject& oJsonObject)
{
    ...
    _cJSON* pJsonStruct = _cJSON_Parse(oJsonObject.ToString().c_str(), &mc_pError) ;
    if (pJsonStruct == NULL)
    {
        m_strErrMsg = std::string("prase json string error at ") + mc_pError;
        return(false);
...
bool CJsonObject::AddWithMove(const std::string& strKey, CJsonObject& oJsonObject)
{
    ...
    _cJSON* pJsonStruct = oJsonObject.m_pJsonData;
    oJsonObject.m_pJsonData = NULL;
    if (pJsonStruct == NULL)
    {
...
bool CJsonObject::Add(const std::string& strKey, const std::string& strValue)
{
    ...
    _cJSON* pJsonStruct = _cJSON_CreateString(strValue.c_str());
    if (pJsonStruct == NULL)
    {
        return(false);
    }
...
bool CJsonObject::Add(const std::string& strKey, int32 iValue)
{
    ...
    _cJSON* pJsonStruct = _cJSON_CreateInt((uint64)iValue, -1);
    if (pJsonStruct == NULL)
    {
        return(false);
    }
...
bool CJsonObject::Add(const std::string& strKey, uint32 uiValue)
{
    ...
    _cJSON* pJsonStruct = _cJSON_CreateInt((uint64)uiValue, 1);
    if (pJsonStruct == NULL)
    {
        return(false);
    }
...
bool CJsonObject::Add(const std::string& strKey, int64 llValue)
{
    ...
    _cJSON* pJsonStruct = _cJSON_CreateInt((uint64)llValue, -1);
    if (pJsonStruct == NULL)
    {
        return(false);
    }
...
bool CJsonObject::Add(const std::string& strKey, uint64 ullValue)
{
    ...
    _cJSON* pJsonStruct = _cJSON_CreateInt(ullValue, 1);
    if (pJsonStruct == NULL)
    {
        return(false);
    }
...
bool CJsonObject::Add(const std::string& strKey, bool bValue, bool bValueAgain)
{
    ...
    _cJSON* pJsonStruct = _cJSON_CreateBool(bValue);
    if (pJsonStruct == NULL)
    {
        return(false);
    }
...
bool CJsonObject::Add(const std::string& strKey, float fValue)
{
    ...
    _cJSON* pJsonStruct = _cJSON_CreateDouble((double)fValue, -1);
    if (pJsonStruct == NULL)
    {
        return(false);
    }
...
bool CJsonObject::Add(const std::string& strKey, double dValue)
{
    ...
    _cJSON* pJsonStruct = _cJSON_CreateDouble((double)dValue, -1);
    if (pJsonStruct == NULL)
    {
        return(false);
    }
...
bool CJsonObject::AddNull(const std::string& strKey)
{
    ...
    _cJSON* pJsonStruct = _cJSON_CreateNull();
    if (pJsonStruct == NULL)
    {
        return(false);
    }
...
bool CJsonObject::Delete(const std::string& strKey)
{
    ...
    _cJSON_DeleteItemFromObject(pFocusData, strKey.c_str());
#if __cplusplus < 201101L
    std::map<std::string, CJsonObject*>::iterator iter = m_mapJsonObjectRef.find(strKey);
#else
    auto iter = m_mapJsonObjectRef.find(strKey);
...
bool CJsonObject::Replace(const std::string& strKey, const CJsonObject& oJsonObject)
{
    ...
    _cJSON_ReplaceItemInObject(pFocusData, strKey.c_str(), pJsonStruct);
    if (_cJSON_GetObjectItem(pFocusData, strKey.c_str()) == NULL)
    {
        return(false);
    }
...
bool CJsonObject::Replace(const std::string& strKey, uint64 ullValue)
{
    ...
    _cJSON* pJsonStruct = _cJSON_CreateInt((uint64)ullValue, 1);
    if (pJsonStruct == NULL)
    {
        return(false);
    }
...
int CJsonObject::GetArraySize()
{
    if (m_pJsonData != NULL)
    {
        if (m_pJsonData->type == _cJSON_Array)
        {
            return(_cJSON_GetArraySize(m_pJsonData));
        }
    }
    else if (m_pExternJsonDataRef != NULL)
    {
        if(m_pExternJsonDataRef->type == _cJSON_Array)
        {
            return(_cJSON_GetArraySize(m_pExternJsonDataRef));
        }
    }
    return(0);
}
...
bool CJsonObject::Get(int iWhich, CJsonObject& oJsonObject) const
{
    ...
    if (m_pJsonData != NULL)
    {
        if (m_pJsonData->type == _cJSON_Array)
        {
            pJsonStruct = _cJSON_GetArrayItem(m_pJsonData, iWhich);
        }
    }
    else if (m_pExternJsonDataRef != NULL)
    {
        if(m_pExternJsonDataRef->type == _cJSON_Array)
        {
            pJsonStruct = _cJSON_GetArrayItem(m_pExternJsonDataRef, iWhich);
        }
    }
    if (pJsonStruct == NULL)
    {
...
bool CJsonObject::Add(const CJsonObject& oJsonObject)
{
    ...
    {
        m_pJsonData = _cJSON_CreateArray();
        pFocusData = m_pJsonData;
    }
    ...
    int iArraySizeBeforeAdd = _cJSON_GetArraySize(pFocusData);
    _cJSON_AddItemToArray(pFocusData, pJsonStruct);
    int iArraySizeAfterAdd = _cJSON_GetArraySize(pFocusData);
    if (iArraySizeAfterAdd == iArraySizeBeforeAdd)
    {
        return(false);
    }
    unsigned int uiLastIndex = (unsigned int)_cJSON_GetArraySize(pFocusData) - 1;
#if __cplusplus < 201101L
    for (std::map<unsigned int, CJsonObject*>::iterator iter = m_mapJsonArrayRef.begin();
...
bool CJsonObject::AddAsFirst(const CJsonObject& oJsonObject)
{
    ...
    int iArraySizeBeforeAdd = _cJSON_GetArraySize(pFocusData);
    _cJSON_AddItemToArrayHead(pFocusData, pJsonStruct);
    int iArraySizeAfterAdd = _cJSON_GetArraySize(pFocusData);
    if (iArraySizeAfterAdd == iArraySizeBeforeAdd)
    {
        return(false);
...
bool CJsonObject::Delete(int iWhich)
{
    ...
    _cJSON_DeleteItemFromArray(pFocusData, iWhich);
#if __cplusplus < 201101L
    for (std::map<unsigned int, CJsonObject*>::iterator iter = m_mapJsonArrayRef.begin();
                    iter != m_mapJsonArrayRef.end(); )
#else
...
bool CJsonObject::Replace(int iWhich, const CJsonObject& oJsonObject)
{
    ...
    _cJSON_ReplaceItemInArray(pFocusData, iWhich, pJsonStruct);
    if (_cJSON_GetArrayItem(pFocusData, iWhich) == NULL)
    {
        return(false);
    }
#if __cplusplus < 201101L
...
bool CJsonObject::Replace(int iWhich, int64 llValue)
{
    ...
    _cJSON* pJsonStruct = _cJSON_CreateInt((uint64)((uint64)llValue), -1);
    if (pJsonStruct == NULL)
    {
        return(false);
    }
...
CJsonObject::CJsonObject(_cJSON* pJsonData)
