...
static dave_bool _uip_server_start_flag = dave_false;
...
_uip_server_http_req(ThreadId src, HTTPRecvReq *pReq)
{
	...
	{
		UIPLOG("listen_port:%d remote_address:%s remote_port:%d recv_fun:%x content:%s",
...
{
	if(_uip_server_start_flag == dave_false)
	{
