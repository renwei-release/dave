...
int sdp_message_v_version_set(sdp_message_t *sdp, char *v_version) {
  ...
  sdp->v_version = osip_strdup(v_version);
  return OSIP_SUCCESS;
}
...
int sdp_message_o_origin_set(sdp_message_t *sdp, char *username, char *sess_id, char *sess_version, char *nettype, char *addrtype, char *addr) {
  ...
  sdp->o_username = osip_strdup(username);
  sdp->o_sess_id = osip_strdup(sess_id);
  sdp->o_sess_version = osip_strdup(sess_version);
  sdp->o_nettype = osip_strdup(nettype);
  sdp->o_addrtype = osip_strdup(addrtype);
  sdp->o_addr = osip_strdup(addr);
  return OSIP_SUCCESS;
}
