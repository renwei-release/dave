...
type VOIPAddCallReq struct {
	Target [DAVE_URL_LEN] byte
	Ptr uint64
}

/* for VOIPMSG_ADD_CALL_RSP message */
type VOIPAddCallRsp struct {
	Ret int64
	Acc_id int64
	Ptr uint64
}
...
type VOIPAnswerCallReq struct {
	Target [DAVE_URL_LEN] byte
	Ptr uint64
}

/* for VOIPMSG_ANSWER_CALL_RSP message */
type VOIPAnswerCallRsp struct {
	Ret int64
	Call_id int64
	Ptr uint64
}
...
type VOIPCFGGetReq struct {
	Ptr uint64
}

/* for VOIPMSG_CFG_GET_RSP message */
type VOIPCFGGetRsp struct {
	Ret int64
	Server_addr [DAVE_URL_LEN] byte
	Server_port uint64
	User [DAVE_USER_NAME_LEN] byte
	Domain [DAVE_DOMAIN_LEN] byte
	Password [DAVE_PASSWORD_LEN] byte
	Impi [DAVE_IMPI_LEN] byte
	Production_environment int8
	Ptr uint64
}

/* for VOIPMSG_CFG_SET_REQ message */
type VOIPCFGSetReq struct {
	Server_addr [DAVE_URL_LEN] byte
	Server_port uint64
	User [DAVE_USER_NAME_LEN] byte
	Domain [DAVE_DOMAIN_LEN] byte
...
type VOIPMakeCallReq struct {
	Conference int8
	Target [DAVE_CONF_USER_MAX*DAVE_URL_LEN] byte
	Ptr uint64
}

/* for VOIPMSG_MAKE_CALL_RSP message */
type VOIPMakeCallRsp struct {
	Ret int64
	Acc_id int64
	Call_id int64
	Ptr uint64
...
type VOIPMuteModeReq struct {
	Mute int8
	Ptr uint64
}

/* for VOIPMSG_MUTE_MODE_RSP message */
type VOIPMuteModeRsp struct {
	Ret int64
	Ptr uint64
}

/* for VOIPMSG_PUSH_TOKEN_SETUP_REQ message */
type VOIPPushTokenSetupReq struct {
	Enable int8
...
type VOIPPushTokenSetupRsp struct {
	Ret int64
	Ptr uint64
}

/* for VOIPMSG_SEND_IM_REQ message */
type VOIPSendIMReq struct {
	Target [DAVE_URL_LEN] byte
	Content [DAVE_IM_LENGTH] byte
	Ptr uint64
}

/* for VOIPMSG_SEND_IM_RSP message */
type VOIPSendIMRsp struct {
	Ret int64
	Ptr uint64
}
