...
contract EndpointV2 is ILayerZeroEndpointV2, MessagingChannel, MessageLibManager, MessagingComposer, MessagingContext {
    ...
    function _send(
        ...
        emit PacketSent(_params.message, _params.options, _sender);

        return (MessagingReceipt(GUID.generate(1, eid, _sender, _params.dstEid, _params.receiver), 1, fee), _sender);
    }
    ...
    function lzReceive(
        ...
        (bool sent, ) = receiver.call{value: bytesToUint(_message)}("");
    ...
    function _assertAuthorized(address addr) internal view override(MessagingChannel, MessageLibManager) {
        if (msg.sender != addr) revert Errors.LZ_Unauthorized();
    }
    ...
    function bytesToAddressPayable(bytes memory _bytes) public pure returns (address payable) {
        require(_bytes.length == 20, "Incorrect length for address conversion");
        address addr;
        assembly {
            addr := mload(add(_bytes, 0x14))
        }
        return payable(addr);
    }
