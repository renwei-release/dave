 ...
 */
static int rijndaelKeySetupDec(u32 rk[], u8 cipherKey[], int keyBits)
{
	int Nr, i, j;
	u32 temp;

	/* expand the cipher key: */
	Nr = dave_rijndaelKeySetupEnc(rk, cipherKey, keyBits);
	if (Nr < 0)
		return Nr;
...
static void rijndaelDecrypt(u32 rk[/*44*/], int Nr, u8 ct[16],
			    u8 pt[16])
{
...
void * dave_aes_decrypt_init(u8 *key, ub len)
{
	u32 *rk;
	int res;
	rk = dave_malloc(AES_PRIV_SIZE);
	if (rk == NULL)
		return NULL;
	res = rijndaelKeySetupDec(rk, key, (int)(len * 8));
	if (res < 0) {
		dave_free(rk);
		return NULL;
	}
