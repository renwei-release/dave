...
static DaveLock _nginx_action_pv;
...
_nginx_action_start(void)
{
	...
	{
		PARTYABNOR("system run %s failed:%d", cmd, ret);
		return ERRCODE_script_execution_error;
	}
...
_nginx_action_safe_start(void)
{
	...
	SAFEZONEv3(_nginx_action_pv, ret = _nginx_action_start(); );
...
_nginx_action_safe_stop(void)
{
	...
	SAFEZONEv3( _nginx_action_pv, ret = _nginx_action_stop(); );
...
_nginx_action_safe_conf_del(ub work_process, ub nginx_port)
{
	...
	SAFEZONEv3( _nginx_action_pv, has_server_number = nginx_conf_del(work_process, nginx_port); );
...
{
	dave_lock_reset(&_nginx_action_pv);
...
{
	SAFEZONEv3( _nginx_action_pv, _nginx_working = dave_true; );
...
{
	SAFEZONEv3( _nginx_action_pv, _nginx_working = dave_false; );
