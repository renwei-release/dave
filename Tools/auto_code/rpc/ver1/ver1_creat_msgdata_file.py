...
def _creat_msgdata_unzip_fun_dimension_object(file_id, struct_type, struct_name, struct_dimension):
    if struct_type == 's8' or struct_type == 'char':
        high_d, high_l = struct_dimension_decomposition(struct_dimension)
        file_id.write("\t\tret = t_rpc_ver1_unzip_"+struct_type
...
def _creat_msgdata_unzip_fun_file(file_id, msg_struct_name, msg_struct_data, struct_table):
    file_id.write("dave_bool\nt_rpc_ver1_unzip_"+msg_struct_name+"(void **unzip_data, ub *unzip_len, void *pStructJson)\n")
    file_id.write("{\n")
    file_id.write("	dave_bool ret;\n")
    file_id.write("\n")
    file_id.write("	if(pStructJson == NULL)\n")
    file_id.write("	{\n")
    file_id.write("		TOOLSLTRACE(360,1,\"the pJson is NULL!\");\n")
...
def _creat_msgdata_inc_file(msg_struct_table, file_list, file_name):
    with open(file_name, "w+", encoding="utf-8") as file_id:
        ...
        for msg_struct_name in msg_struct_table.keys():
            ...
            file_id.write("dave_bool t_rpc_ver1_unzip_"+msg_struct_name+"(void **unzip_data, ub *unzip_len, void *pStructJson);\n\n")
...
def creat_msgdata_file():
    ...
    print(f"{len(msg_struct_table)}\tmsgdata\t\twrite to {rpc_ver1_msgdata_src_file_name}")
    _creat_msgdata_src_file(msg_struct_table, msg_file_list, rpc_ver1_msgdata_src_file_name)
    _creat_msgdata_inc_file(msg_struct_table, msg_file_list, rpc_ver1_msgdata_inc_file_name)
    return