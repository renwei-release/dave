...
def _write_exist_enum(msg_id_exist_table, file_id):
    ...
    for key in msg_id_exist_table:
        file_id.write(f'\t{msg_id_exist_table[key]},\n')

    if key == None:
        return 1
    msg_id_value = re.findall('[A-Z,a-z,0-9,_]* = ([0-9]*)', msg_id_exist_table[key])
    return int(msg_id_value[0]) + 1
...
def creat_rpcinc_file():
    ...
    print(f"{len(msg_id_list)}\tmsgid\t\twrite to {ver3_rpcinc_file_name}")
    _creat_rpcinc_file(msg_id_list, ver3_rpcinc_file_name)
    return