...
def _load_picture(picture, preprocess):  # 输入图像文字数据(图片skimage自带)
    original_images = []
    images = []
    if type(picture).__name__ == 'bytes':
        image = Image.open(io.BytesIO(picture))
    elif type(picture).__name__ == 'str':
...
def _feature_normalization(images, model):
...
class predict():
    def __init__(self, label = None,model_file = None):
        if label == None:
        ...
        self.model, self.preprocess = _load_model(model_file)
        self.text_tokens, self.ccll = _load_label(label)

    def predict(self, image_info=None, top=3):
        self.images = _load_picture(image_info, self.preprocess)
        self.image_features = _feature_normalization(self.images, self.model)
        return _model_predict(self.image_features, self.model, self.text_tokens, self.ccll, top)

    def feature(self, image_info=None, top=5):
        out_label, out_score = self.predict(image_info,top)
