...
def write_msg(dst, msg_id, class_instance):
    ...
    if isinstance(dst, c_char) or isinstance(dst, c_char_p) or isinstance(dst, str) or isinstance(dst, bytes):
        davelib.dave_thread_name_msg(c_char_p(dst), c_longlong(msg_id), c_int(sizeof(class_instance.contents)), class_instance, c_char_p(__func__), c_int(__LINE__))
    else:
        davelib.dave_thread_id_msg(c_uint64(-1), c_uint64(dst), c_int(0), c_longlong(msg_id), c_uint64(sizeof(class_instance.contents)), class_instance, c_char_p(__func__), c_int(__LINE__))
    return
